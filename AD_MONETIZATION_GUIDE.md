...
8. [注意事项](#8-注意事项)
9. [行动清单](#9-行动清单)
10. [综合对比与最终推荐](#10-综合对比与最终推荐)
11. [广告之外的变现方式](#11-广告之外的变现方式)
12. [遗产通现有变现基础设施](#12-遗产通现有变现基础设施)
...
| 平台 | eCPM (Banner/插屏/激励视频) | 日活门槛 | 审核周期 | 结算周期 | SDK大小 | 对法律类App友好度 | 综合推荐指数 |
|------|----------------------------|----------|----------|----------|---------|-------------------|--------------|
| **穿山甲** | ¥5–15 / ¥30–80 / ¥60–150 | 建议 500+ | 1–3 工作日 | 月结，满 ¥100 提现 | ~5 MB | ★★★★★ | ★★★★★ |
| **优量汇** | ¥4–12 / ¥25–65 / ¥50–120 | 无硬性要求 | 约 1 工作日 | 月结，满 ¥100 提现 | ~4 MB | ★★★★☆ | ★★★★☆ |
| **快手联盟** | ¥4–12 / ¥25–70 / ¥55–130 | 建议 1000+ | 2–5 工作日 | 月结 | ~6 MB | ★★★☆☆ | ★★★☆☆ |
| **百度联盟** | ¥3–10 / ¥20–60 / ¥45–100 | 无硬性要求 | 2–5 工作日 | 月结 | ~5 MB | ★★★☆☆ | ★★★☆☆ |
| **华为广告** | ¥4–12 / ¥25–70 / ¥50–120 | 无硬性要求 | 1–3 工作日 | 月结 | ~5 MB | ★★★★☆ | ★★★☆☆ |
| **AdMob** | 海外高，国内受限 | 无硬性要求 | 即时 | 月结，$100 起 | ~3 MB | ★★☆☆☆ | ★★☆☆☆ |

### 10.2 最终推荐及理由

**PRIMARY（首选）：穿山甲**

- **eCPM 最高**：Banner、插屏、激励视频均处于行业领先水平
- **库存最大**：字节系（抖音、今日头条等）广告主投放量大，填充率好
- **文档最全**：接入文档、测试工具、FAQ 完善，适合零经验开发者
- **市场领先**：国内广告联盟市占率约 40%，生态成熟
- **法律类 App 友好**：工具/教育类应用审核通过率高，无特殊限制

**SECONDARY（备选）：优量汇**

- **腾讯生态**：微信、QQ 用户覆盖广，适合内容型应用
- **门槛较低**：个人开发者友好，审核相对宽松
- **适合内容 App**：与遗产通「法律百科」定位契合

**为何不选其他平台（针对遗产通）**

| 平台 | 不选原因 |
|------|----------|
| 快手联盟 | 日活门槛高（1000+），遗产通初期难以达标；偏娱乐场景，法律类填充一般 |
| 百度联盟 | eCPM 中等，文档与支持不如穿山甲；可作为 Phase 3 第三备选 |
| 华为广告 | 仅覆盖华为设备用户，遗产通全渠道分发时占比有限；华为渠道用户多时可考虑 |
| AdMob | 国内需 VPN，用户覆盖受限；若有海外版本再考虑 |

---

## 11. 广告之外的变现方式

| 方式 | 预期收入 | 实现难度 | 适合阶段 | 具体做法 |
|------|----------|----------|----------|----------|
| **法律咨询引流** | 每单 ¥50–200 佣金，月入数百至数千 | 中 | DAU 1000+ | 接入律师平台 API（如华律、找法网），用户点击「联系律师」时跳转并记录来源；或自建表单收集需求，对接律所按线索结算 |
| **知识付费** | 单份 ¥9.9–49.9，月入数百 | 中 | DAU 500+ | 提供「深度案例解读报告」「遗嘱模板包」「继承流程图」等付费内容；接入微信支付/支付宝 |
| **增值服务** | 单次 ¥2–10，月入数百 | 中高 | DAU 1000+ | 文档生成（遗嘱草稿、继承清单导出）、法律意见书 PDF 导出；按次或包月收费 |
| **打赏/赞助** | 月入数十至数百 | 低 | 随时 | 在「关于」页增加「支持开发者」按钮，跳转微信/支付宝收款码或爱发电等平台 |
| **企业合作** | 年费 ¥5,000–50,000 | 高 | DAU 5000+ | 与律所、公证处合作，在 App 内提供「推荐律所」「公证预约」等入口，收取展示费或按转化分成 |

---

## 12. 遗产通现有变现基础设施

本节记录遗产通 App 中已实现的变现相关基础设施，便于后续接入广告 SDK 时复用。

### 12.1 Analytics 框架（含 GitHub Gist 上传）

- **位置**：`com.jichengtong.app.utils.Analytics`
- **功能**：事件埋点（启动、页面浏览、搜索、内容查看、AI 提问、收藏、分享）、本地事件日志、完整报告导出
- **开发测试**：`AnalyticsDashboardActivity` 提供「上传到 GitHub Gist」功能，配置 GitHub Token 后可将设备数据上传为私有 Gist，便于远程查看测试数据
- **用途**：变现前可验证用户行为路径；接入广告后可增加 `ad_banner_request`、`ad_interstitial_request` 等事件追踪

### 12.2 RemoteConfig 广告控制

- **位置**：`com.jichengtong.app.utils.RemoteConfig`，配置源 `remote_config.json`
- **Schema**：`ad_config` 已完整定义，支持：
  - `enabled`：全局开关
  - `provider`：`pangle` / `gdt` / `none`
  - `banner`、`interstitial`、`rewarded`、`native`：各广告位开关与 `unit_id`
  - `frequency_minutes`：插屏频次控制
  - `new_user_ad_free_days`：新用户免广告天数
- **用途**：上线前关闭所有广告，审核通过后远程开启；可灰度、A/B 测试

### 12.3 AdManager 框架（SDK 接入就绪）

- **位置**：`com.jichengtong.app.utils.AdManager`
- **架构**：无 SDK 时为空实现；接入穿山甲/优量汇后，在 `loadBanner`、`showInterstitial`、`showRewardedVideo` 中替换为真实 SDK 调用
- **已实现**：从 RemoteConfig 读取配置、新用户免广告、插屏频次控制、埋点上报
- **待接入**：穿山甲/优量汇 Gradle 依赖及具体 `loadXxx` 实现

### 12.4 广告位规划与架构

- **已规划**：首页 Banner、案例详情原生、AI 对话后插屏、法条详情 Banner、激励视频解锁（见第 5 节）
- **布局**：各页面已预留或可快速插入广告容器
- **架构**：AdManager 统一入口，RemoteConfig 控制展示逻辑，无需改动业务代码即可开关广告

### 12.5 开发者数据看板

- **入口**：`AnalyticsDashboardActivity`（「我的」→ 数据看板，或 Debug 菜单）
- **功能**：事件统计展示、事件日志查看、JSON 报告复制/分享、GitHub Gist 上传、广告状态实时展示（从 AdManager 读取）
- **用途**：开发阶段验证埋点与广告配置；上线后可作为内部监控入口
